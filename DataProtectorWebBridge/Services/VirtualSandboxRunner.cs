...
namespace DataProtectorWebBridge.Services
{
    internal sealed class VirtualSandboxRunner
    {
            ...
            string signer)
        {
            ...
            script.AppendLine("$localTelemetryRoot = Join-Path $workspace 'Telemetry'");
            script.AppendLine("$reportPath = Join-Path $reportRoot 'report.json'");
            script.AppendLine("$donePath = Join-Path $reportRoot 'report.done'");
            script.AppendLine("$stdoutPath = Join-Path $reportRoot 'telemetry.stdout.log'");
            script.AppendLine("$stderrPath = Join-Path $reportRoot 'telemetry.stderr.log'");
            script.AppendLine("function Write-FailureReport([string]$stage, [string]$message, [int]$exitCode){ $payload = [ordered]@{ schema='dataprotector.sandbox.report.v2'; error='Telemetry runner failed'; stage=$stage; message=$message; exitCode=$exitCode; runner=$runner; runtime=$runtime; kernelDriver=$kernelDriver; policyApi=$policyApi; runPath=$runPath; workspace=$workspace; telemetryRoot=$telemetryRoot; localTelemetryRoot=$localTelemetryRoot; runnerExists=(Test-Path -LiteralPath $runner); runtimeExists=(Test-Path -LiteralPath $runtime); kernelDriverExists=(Test-Path -LiteralPath $kernelDriver); policyApiExists=(Test-Path -LiteralPath $policyApi); sampleExists=(Test-Path -LiteralPath $runPath); stdout=($(if(Test-Path -LiteralPath $stdoutPath){ Get-Content -LiteralPath $stdoutPath -Raw } else { '' })); stderr=($(if(Test-Path -LiteralPath $stderrPath){ Get-Content -LiteralPath $stderrPath -Raw } else { '' })) }; $json = $payload | ConvertTo-Json -Depth 6 -Compress; [System.IO.File]::WriteAllText($reportPath, $json, [System.Text.UTF8Encoding]::new($false)); [System.IO.File]::WriteAllText($donePath, 'done', [System.Text.UTF8Encoding]::new($false)) }");
            script.AppendLine("New-Item -ItemType Directory -Force -Path $workspace, $reportRoot | Out-Null");
            script.AppendLine("try { if($copyInputDirectory){ Copy-Item -Path (Join-Path $inputRoot '*') -Destination $workspace -Recurse -Force } else { Copy-Item -LiteralPath (Join-Path $inputRoot $sampleName) -Destination (Join-Path $workspace $sampleName) -Force } } catch { $runPath = Join-Path $workspace $sampleName; $runner=''; $runtime=''; $kernelDriver=''; $policyApi=''; Write-FailureReport 'copy-input' $_.Exception.Message -1; exit 1 }");
            script.AppendLine("try { Copy-Item -LiteralPath $telemetryRoot -Destination $localTelemetryRoot -Recurse -Force } catch { $runPath = Join-Path $workspace $sampleName; $runner=''; $runtime=''; $kernelDriver=''; $policyApi=''; Write-FailureReport 'copy-telemetry' $_.Exception.Message -1; exit 1 }");
            script.AppendLine("$runPath = Join-Path $workspace $sampleName");
            script.AppendLine("$runner = Join-Path $localTelemetryRoot 'DataProtectorSandboxTelemetry.exe'");
            script.AppendLine("$runtime = Join-Path $localTelemetryRoot 'x64\\DataProtectorUserHookRuntime.dll'");
            script.AppendLine("$kernelDriver = Join-Path $localTelemetryRoot 'kernel\\DataProtector.sys'");
            script.AppendLine("$policyApi = Join-Path $localTelemetryRoot 'kernel\\DataProtectorPolicyApi.dll'");
            script.AppendLine("if($sampleArchitecture -eq 'x86'){ $runnerCandidate = Join-Path $localTelemetryRoot 'x86\\DataProtectorSandboxTelemetry.exe'; if(Test-Path -LiteralPath $runnerCandidate){ $runner = $runnerCandidate }; $runtime = Join-Path $localTelemetryRoot 'x86\\DataProtectorUserHookRuntime.dll'; $policyApiCandidate = Join-Path $localTelemetryRoot 'kernel\\x86\\DataProtectorPolicyApi.dll'; if(Test-Path -LiteralPath $policyApiCandidate){ $policyApi = $policyApiCandidate } }");
            script.AppendLine("if(!(Test-Path -LiteralPath $runner)){ Write-FailureReport 'runner-missing' 'DataProtectorSandboxTelemetry.exe was not copied into the sandbox local workspace.' -1; exit 1 }");
            script.AppendLine("$runnerArgs = @('--run-id', $runId, '--sample', $runPath, '--report', $reportPath, '--runtime', $runtime, '--kernel-driver', $kernelDriver, '--policy-api', $policyApi, '--timeout', ([string]$timeoutSeconds), '--network', ([string]$networkEnabled))");
            script.AppendLine("if(-not [string]::IsNullOrWhiteSpace($argumentText)){ $runnerArgs += @('--arguments', $argumentText) }");
            script.AppendLine("try { $runnerProcess = Start-Process -FilePath $runner -ArgumentList $runnerArgs -WorkingDirectory $workspace -RedirectStandardOutput $stdoutPath -RedirectStandardError $stderrPath -Wait -PassThru; if(!(Test-Path -LiteralPath $donePath)){ Write-FailureReport 'runner-no-report' 'Telemetry runner exited without writing report.done.' $runnerProcess.ExitCode } } catch { Write-FailureReport 'runner-start' $_.Exception.ToString() -1 }");
